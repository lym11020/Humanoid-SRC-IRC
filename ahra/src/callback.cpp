...
void Callback::StartMode(const std_msgs::Bool::ConstPtr &start)
{
    if (start->data)
    {
        ...
        // flgflg = 1;
        // srv_SendMotion.response.select_motion = Motion_Index::ForWard_fast4step; //332

        // emergency = 0;
        // srv_SendMotion.request.TA_finish = true;
        // ROS_INFO("Huddle_Jump!");
    ...
    else // 여기서 else if 대신 else를 사용하여 start->data가 거짓인 경우를 처리합니다.
    {
        ...
        flgflg = 1;
        srv_SendMotion.response.select_motion = Motion_Index::Forward_2step; //402 ->198
        emergency = 0;
        srv_SendMotion.request.TA_finish = true;
        ROS_INFO("Huddle_Jump!");
    }
...
void Callback::SelectMotion()
{
    ...
    else if (res_mode == Motion_Index::Huddle_Jump)
    {
        ...
        // trajectoryPtr->Huddle_Motion(0.2, 0.14, 0.05);
        IK_Ptr->Get_Step_n(trajectoryPtr->Return_Step_n());                        
        IK_Ptr->Change_Angle_Compensation(6.5, 3.5, 0, 3.5, 6.5, 3.5, 0, -3.5);
        // IK_Ptr->Change_Angle_Compensation(5.5, 4.5, 0, 4.5, 5.5, 4.5, 0, -4.5);
        IK_Ptr->Set_Angle_Compensation(135);
        indext = 0; 

        //         IK_Ptr->Change_Com_Height(30);
        // // trajectoryPtr->Test_Motion(0.22, 0.14, 0.05);
        // trajectoryPtr->Test_Motion(0.2, 0.14, 0.05);
        // IK_Ptr->Get_Step_n(trajectoryPtr->Return_Step_n());                        
        // // IK_Ptr->Change_Angle_Compensation(6.5, 3.5, 0, 3.5, 6.5, 3.5, 0, -3.5);
        // IK_Ptr->Change_Angle_Compensation(5.5, 4.5, 0, 4.5, 5.5, 4.5, 0, -4.5);
        // IK_Ptr->Set_Angle_Compensation(135);
        ...
        // {
        //     fprintf(Trajectory_all, "%d ", i);
        //     fprintf(Trajectory_all, "%lf ", trajectoryPtr->Ref_LL_x(i));
        //     fprintf(Trajectory_all, "%lf ", trajectoryPtr->Ref_LL_y(i));
        //     fprintf(Trajectory_all, "%lf ", trajectoryPtr->Ref_LL_z(i));
        //     fprintf(Trajectory_all, "%lf ", trajectoryPtr->Ref_RL_x(i));
        //     fprintf(Trajectory_all, "%lf ", trajectoryPtr->Ref_RL_y(i));
        //     fprintf(Trajectory_all, "%lf ", trajectoryPtr->Ref_RL_z(i));
        //     fprintf(Trajectory_all, "\n ");
        // }
    }

    else if (res_mode == Motion_Index::Test)
    {
        mode = Motion_Index::Test;
        IK_Ptr->Change_Com_Height(30);
        // trajectoryPtr->Test_Motion(0.22, 0.14, 0.05);
        trajectoryPtr->Test_Motion(0.2, 0.14, 0.05);
        IK_Ptr->Get_Step_n(trajectoryPtr->Return_Step_n());                        
...
void Callback::Write_Leg_Theta()
{
    ...
    if (emergency == 0)
    {
        ...
        else if (mode == Motion_Index::Huddle_Jump)
        {
            ...
            // IK_Ptr->Angle_Compensation_Huddle(indext);
                        IK_Ptr->Angle_Compensation_test(indext);

        }


        else if (mode == Motion_Index::Test)
        {
    ...
    All_Theta[3] = -IK_Ptr->RL_th[3] + 45 * DEG2RAD; //38.34 * DEG2RAD;
...
void Callback::Write_Arm_Theta()
{
    ...
    if (upper_indext >= trajectoryPtr->Ref_RL_x.cols() - 2 && upper_indext != 0)
    {
            upper_indext -= 1;
            // srv_SendMotion.request.SM_finish = true;
    ...
    All_Theta[13] = 90 * DEG2RAD; //-90 * DEG2RAD;
    All_Theta[14] = -90 * DEG2RAD; //90 * DEG2RAD;
...
void Callback::Write_All_Theta()
{
    ...
    //upper_body
    All_Theta[12] = pick_Ptr->WT_th[0] + 0 * DEG2RAD;  //waist
    ...
    All_Theta[22] = pick_Ptr->NC_th[1] + 0 * DEG2RAD; //neck_UD
