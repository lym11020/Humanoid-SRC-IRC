...
MatrixXd Trajectory::Huddle_Xcom()
{
	...
	for (int i = 0; i < sim_n + 19; i++)
	{
		...
		else if (i < 4.5 * float_walktime_n) //4.5
		{
			zmp_ref[i] = step;
		}
		else
...
MatrixXd Trajectory::test_Ycom()
{
	...
	for (int i = 0; i < sim_n + 19; i++)
	{
		...
		else if (i < 1.75 * float_walktime_n) //1.75
		{
			zmp_ref[i] = -0.07;
		}
		else if (i < 3.0 * float_walktime_n) //4.5
		{
			zmp_ref[i] = 0.08; //0.06
		}
		else if (i < 5.7 * float_walktime_n)
...
// 			zmp_ref[i] =  step;
// 		}		
// 		// else if (i < 5 * float_walktime_n)
// 		// {
// 		// 	zmp_ref[i] = step;
// 		// }
// 		// else if (i < 5.5 * float_walktime_n)
// 		// {
// 		// 	zmp_ref[i] = step;
// 		// }
// 		else
...
// 			zmp_ref[i] = -0.07;
// 		}
// 		else if (i < 3.0 * float_walktime_n)
// 		{
// 			zmp_ref[i] = 0.08; //0.06
// 		}
// 		// else if (i < 5.7 * float_walktime_n)
...
void Trajectory::Test_Motion(double step, double height, double COM_h)
{
	Set_step(step);
	MatrixXd Ref_RL_x_t = RF_xsimulation_test() - test_Xcom();
	MatrixXd Ref_LL_x_t = LF_xsimulation_test() - test_Xcom();
	MatrixXd Ref_RL_y_t = -L0 * MatrixXd::Ones(1, sim_n) - test_Ycom();
	MatrixXd Ref_LL_y_t = L0 * MatrixXd::Ones(1, sim_n) - test_Ycom();
	MatrixXd Ref_RL_z_t = RF_zsimulation_test(height, COM_h);
	MatrixXd Ref_LL_z_t = LF_zsimulation_test(height, COM_h);
	MatrixXd Temp_RL_x(Ref_RL_x_t.rows(), MatrixXd::Zero(1, 100).cols() + Ref_RL_x_t.cols());
	MatrixXd Temp_LL_x(Ref_LL_x_t.rows(), MatrixXd::Zero(1, 100).cols() + Ref_LL_x_t.cols());
	MatrixXd Temp_RL_y(Ref_RL_y_t.rows(), MatrixXd::Ones(1, 100).cols() + Ref_RL_y_t.cols());
	MatrixXd Temp_LL_y(Ref_LL_y_t.rows(), MatrixXd::Ones(1, 100).cols() + Ref_LL_y_t.cols());
	MatrixXd Temp_RL_z(Ref_RL_z_t.rows(), RF_zsimulation_sitdown(COM_h).cols() + Ref_RL_z_t.cols());
	MatrixXd Temp_LL_z(Ref_LL_z_t.rows(), LF_zsimulation_sitdown(COM_h).cols() + Ref_LL_z_t.cols());
	Temp_RL_x << MatrixXd::Zero(1, 100), Ref_RL_x_t;
	Temp_LL_x << MatrixXd::Zero(1, 100), Ref_LL_x_t;
	Temp_RL_y << -L0 * MatrixXd::Ones(1, 100), Ref_RL_y_t;
	Temp_LL_y << L0 * MatrixXd::Ones(1, 100), Ref_LL_y_t;
	Temp_RL_z << RF_zsimulation_sitdown(COM_h), Ref_RL_z_t;
	Temp_LL_z << LF_zsimulation_sitdown(COM_h), Ref_LL_z_t;
	Ref_RL_x = Temp_RL_x;
	Ref_LL_x = Temp_LL_x;
	Ref_RL_y = Temp_RL_y;
	Ref_LL_y = Temp_LL_y;
...
// void Trajectory::Sitdown_Motion(double COM_h)
// {
// 	Ref_RL_x = MatrixXd::Zero(1, 100);
// 	Ref_LL_x = MatrixXd::Zero(1, 100);
// 	Ref_RL_y = -L0 * MatrixXd::Ones(1, 100);
// 	Ref_LL_y = L0 * MatrixXd::Ones(1, 100);
// 	Ref_RL_z = RF_zsimulation_sitdown(COM_h);
// 	Ref_LL_z = LF_zsimulation_sitdown(COM_h);
// }

// void Trajectory::Standup_Motion(double COM_h)
// {
// 	Ref_RL_x = MatrixXd::Zero(1, 100);
// 	Ref_LL_x = MatrixXd::Zero(1, 100);
// 	Ref_RL_y = -L0 * MatrixXd::Ones(1, 100);
// 	Ref_LL_y = L0 * MatrixXd::Ones(1, 100);
// 	Ref_RL_z = RF_zsimulation_standup(COM_h);
// 	Ref_LL_z = LF_zsimulation_standup(COM_h);
// }
...
void Pick::WT_Trajectory(double WT_th, int length)
{
    ...
    {
        double t = del_t * (i + 1);  
        WT(i) = WT_th * (1 - cos(PI * t / (del_t * length)));
    }

    Ref_WT_th.resize(1, length);  
    Ref_WT_th.row(0) = WT * deg2rad;
}
...
void Pick::RA_Trajectory(double RA_th1, double RA_th2, double RA_th3, double RA_th4, int length)
{
    ...
    {
        double t = del_t * (i + 1); 
        RA_1(i) = RA_th1 * (1 - cos(PI * t / (del_t * length)));
        RA_2(i) = RA_th2 * (1 - cos(PI * t / (del_t * length)));
        RA_3(i) = RA_th3 * (1 - cos(PI * t / (del_t * length)));
        RA_4(i) = RA_th4 * (1 - cos(PI * t / (del_t * length)));
    }

    Ref_RA_th.resize(4, length);  
    Ref_RA_th.row(0) = RA_1 * deg2rad;
    Ref_RA_th.row(1) = RA_2 * deg2rad;
    Ref_RA_th.row(2) = RA_3 * deg2rad;
    Ref_RA_th.row(3) = RA_4 * deg2rad;
}
...
void Pick::LA_Trajectory(double LA_th1, double LA_th2, double LA_th3, double LA_th4, int length)
{
    ...
    {
        double t = del_t * (i + 1);  
        LA_1(i) = LA_th1 * (1 - cos(PI * t / (del_t * length)));
        LA_2(i) = LA_th2 * (1 - cos(PI * t / (del_t * length)));
        LA_3(i) = LA_th3 * (1 - cos(PI * t / (del_t * length)));
        LA_4(i) = LA_th4 * (1 - cos(PI * t / (del_t * length)));
    }

    Ref_LA_th.resize(4, length);
    Ref_LA_th.row(0) = LA_1 * deg2rad;
    Ref_LA_th.row(1) = LA_2 * deg2rad;
    Ref_LA_th.row(2) = LA_3 * deg2rad;
    Ref_LA_th.row(3) = LA_4 * deg2rad;
}
...
void Pick::NC_Trajectory(double NC_th1, double NC_th2, int length)
{
    ...
    for (int i = 0; i < length; ++i)
    {
        double t = del_t * (i + 1);  // t = del_t * (i + 1)
        NC_1(i) = NC_th1 * (1 - cos(PI * t / (del_t * length)));
        NC_2(i) = NC_th2 * (1 - cos(PI * t / (del_t * length)));
    }

    Ref_NC_th.resize(2, length);
    Ref_NC_th.row(0) = NC_1 * deg2rad;
    Ref_NC_th.row(1) = NC_2 * deg2rad;
}
...
void Pick::UPBD_SET(const MatrixXd& WT, const MatrixXd& RA, const MatrixXd& LA, const MatrixXd& NC, int Index_CNT)
{
    ...
    Index_CNT = std::min(Index_CNT, static_cast<int>(WT.cols()) - 1);
